...
// task 2a isClassExp to use in parsing
...
const isSpecialForm = (x: string): boolean =>
    ["class", "if", "lambda", "let", "quote"].includes(x); // I have added "class" task 2a
...
export const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    ...
    const fieldDecls: VarDecl[] = fields.map(makeVarDecl);
    
    
    if (!isArray(methods) || !allT(isNonEmptyList, methods)) {
        return makeFailure('Malformed methods in "class" expression');
    }

    const parsedMethods: Result<Binding[]> = parseMethods(methods); // helper function for parsing the methods
    
    
    return bind(parsedMethods, (validParsedMethods: Binding[]) =>
        makeOk(makeClassExp(fieldDecls, validParsedMethods))
    );
};
...
const parseMethods = (methods: Sexp): Result<Binding[]> => {
    if (!isGoodBindings(methods)) {
        return makeFailure('Malformed methods in "Class" expression');
    }
    const names = map(b => b[0], methods);
    const valsResult = mapResult(parseL3CExp, map(second, methods));
    const methodsResult = mapv(valsResult, (vals: CExp[]) => zipWith(makeBinding, names , vals));
...
const unparseClassExp = (exp: ClassExp): string => {
    const fields = exp.fields.map(field => field.var).join(' ');
    const methods = exp.methods.map((method: Binding) => `(${method.var.var} ${unparseL3(method.val)})`).join(" ");
