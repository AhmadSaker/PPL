...
import { isClosure, makeClosureEnv, Closure, Value, makeClassValue, makeClosure, ClassValue, makeObjectValue, ObjectValue, isClassValue, isObjectValue, isSymbolSExp } from "./L3-value";
...
const applyClass = (cls: ClassValue, args: Value[]): Result<Value> => { 
    if (!isNonEmptyList<Value>(args)){
        return makeFailure('fields needed to create an object')
    }
    return makeOk(makeObjectValue(args, cls.methods, makeExtEnv(cls.fields.map((v : VarDecl)=>v.var), args, cls.env)));
};

export const applyObject = (obj: ObjectValue, args: Value[]): Result<Value> => {
    if (!isNonEmptyList<Value>(args))  return makeFailure ('nothing given to do') 
    if (!isSymbolSExp(args[0]))  return makeFailure('this is not a method') 
    ...
    return bind(applicativeEval(methodExp, obj.env), (methodValue: Value) => {
        
        if (isClosure(methodValue)) {
            return applyClosure(methodValue, obj.fields.concat(args));
        } else {
