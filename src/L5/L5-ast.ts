...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makeTypePredTExp } from './TExp';
...
// (define [Cf : [number * number -> number]] (lambda ([x : number] [y : number]) : number (* x x))
// (define f (lambda ([x : number]) (* x x)))  // no type annotation on f and on return value of lambda
// (let (([a : number] 1)
//       ([b : boolean] #t))
//   (if b a (+ a 1)))
// (define [id : (T1 -> T1)] (lambda ([x : T1]) : T1 x))
//      L52 - annotation for type predicate
// (define [is_number : [any -> is number]] (lambda ([x : any]) : is number (number? x))
;;
...
//         |  <var-ref>
//         |  ( lambda ( <var-decl>* ) [: is? <TExp>]? <cexp>+ ) / ProcExp(args:VarDecl[], body:CExp[], returnTE: TExp))
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const isTypePred = rest[0] === "is?";
        const returnTE = isTypePred ? mapv(parseTExp(rest[1]), (te: TExp) => makeTypePredTExp(te)) :
                        (rest[0] === ":" ? parseTExp(rest[1]) : makeOk(makeFreshTVar()));
        const body = mapResult(parseL5CExp, isTypePred ? rest.slice(2) : (rest[0] === ":" ? rest.slice(2) : rest));
