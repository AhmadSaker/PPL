...
(isTVar(te1) || isTVar (te2)) ? makeOk(true) : 
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<TEnv[]> => {
    ...
    const rator = e.rator;
    if (!isVarRef(rator)) {
        return makeFailure(`rator is not a variable reference`);
    }
    const ratorType = applyTEnv(tenv, rator.var);
    return bind(ratorType, (ratorTE: TExp) => {
        if (isTypePredTExp(ratorTE)) {
            const arg = e.rands[0];
            const argType = typeofExp(arg, tenv);
            return bind(argType, (argTE: TExp) => {
                const trueEnv = makeExtendTEnv([rator.var], [ratorTE.type], tenv);
                const falseEnv = makeExtendTEnv([rator.var], [makeDiffTExp(argTE, ratorTE.type)], tenv);
                return makeOk([trueEnv, falseEnv]);
            });
        } else {
            return makeFailure(`rator is not a type predicate`);
        }
    });
};
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
        either(bind(isTypePredApp(ifExp.test, tenv), (newEnvs: TEnv[]) => {
            return bind(typeofExp(ifExp.then, newEnvs[0]), (then: TExp) =>
                bind(typeofExp(ifExp.alt, newEnvs[1]), (alt: TExp) =>
                    makeOk(makeUnion(then, alt))));
        }),
            makeOk, () => typeofIfNormal(ifExp, tenv));
...
export const typeOfPredProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    ...
    const extTEnv = makeExtendTEnv(map((vd) => vd.var, proc.args), argsTEs, tenv);
    const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) =>
        isBoolTExp(body) ? makeOk(true) : bind(unparseTExp(body), (body: string) => makeFailure(`Type of body must be bool for type predicate! got type: ${body}`)));
    return bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE)));
}
...
export const typeofApp = (appExp: AppExp, typeEnv: TEnv): Result<TExp> =>
    bind(typeofExp(appExp.rator, typeEnv), (ratorTypeExp: TExp) => {
        if (!isProcTExp(ratorTypeExp)) {
            return bind(unparseTExp(ratorTypeExp), (unparsedRator: string) =>
                bind(unparse(appExp), (unparsedAppExp: string) =>
                    makeFailure<TExp>(`Application of non-procedure: ${unparsedRator} in ${unparsedAppExp}`)));
        }
        if (appExp.rands.length !== ratorTypeExp.paramTEs.length) {
            return bind(unparse(appExp), (unparsedAppExp: string) => 
                makeFailure<TExp>(`Wrong number of parameters passed to procedure: ${unparsedAppExp}`));
        }
        const paramChecks = zipWithResult(
            (rand, paramType) => bind(typeofExp(rand, typeEnv), (randType: TExp) =>
                checkCompatibleType(randType, paramType, appExp)),
            appExp.rands, ratorTypeExp.paramTEs
        );
        return bind(paramChecks, _ => makeOk(ratorTypeExp.returnTE));
    });
